...
import { FileTypeParser, reasonableDetectionSizeInBytes } from "./core";

export class NodeFileTypeParser extends FileTypeParser {
  async fromStream(stream) {
    const tokenizer = await (stream instanceof ReadableStream
...
export async function fileTypeFromFile(path, fileTypeOptions) {
  return new NodeFileTypeParser(fileTypeOptions).fromFile(
    path,
    fileTypeOptions,
  );
}
...
export async function fileTypeStream(readableStream, options = {}) {
  return new NodeFileTypeParser(options).toDetectionStream(
    readableStream,
    options,
  );
}
